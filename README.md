...
- [x] Phase 4 — Planner + Tool use 결합한 멀티홉 에이전트
- [ ] Phase 5 — 채점 파이프라인 구축 + ablation 실험

### 알려진 한계

- NAVER 뉴스 검색은 인기 검색어일수록 `sort=date`/`sim` 둘 다 최근 며칠치로 결과가 꽉 차서, 몇 달 전 과거 기사까지는 사실상 도달하지 못함 (직접 확인). 이벤트 특화 표현("실적 발표", "컨퍼런스콜")이 일반 단어보다 더 과거까지 도달하지만 회사/표현마다 편차가 큼 — 완전한 해결책은 아님.
- DART에는 주가 데이터가 없음(재무제표만 있음). "왜 주가가 떨어졌어" 같은 질문은 지금 도구(DART+뉴스) 조합만으론 원천적으로 답할 수 없음 — 필요해지면 주가 시계열 API를 세 번째 도구로 추가해야 함.
